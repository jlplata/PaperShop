...
namespace PaperShop
{
    public partial class frmSalidas : Form
    {
        ...
        void ComboCliente()
        {
            ...
            try
            {
                qry = "Select id_cliente, numero_cliente as cliente from clientes where activo=1";
                //Asignamos la consulta al comando
                sqlCMD.CommandText = qry;
                //Asignamos la conexion al comando
                sqlCMD.Connection = sqlCNX;
                ...
                cmbcliente.AutoCompleteCustomSource = coleccion;
        ...
        void idsalida()
        {
            ...
            qry = "SELECT id_salida from salidas where fecha_registro='" + this.lblfecha_registro.Text + "' and hora_registro='" + this.lblhora_registro.Text + "'";
            //Asignamos la consulta al comando 
            sqlCMD.CommandText = qry;
            //Asignamos la conexion al comando
            sqlCMD.Connection = sqlCNX;
            ...
            try
            {
                ...
                if (sqlDR.HasRows == true)
                {
                    ...
                    {
                        this.txtidsalida.Text = sqlDR["id_salida"].ToString();
                    }
                    //Cerramos la conexion
                    sqlCNX.Close();
                }
            ...
            {
                MessageBox.Show("Error al extraer el id de la salida", "Sistema de inventario");
        ...
        void AgregarProducto()
        {
            ...
            if (this.txtcve_producto.Text.Length > 0 && this.txtcantidad.Text.Length > 0)
            {
                ...
                qry = "SELECT id_producto, nombre, cantidad, precio_venta from productos where cve_producto='" + this.txtcve_producto.Text + "'";
                //Asignamos la consulta al comando
                sqlCMD.CommandText = qry;
                //Asignamos la consulta al comando
                sqlCMD.Connection = sqlCNX;
        ...
        private void Btnregistrar_salida_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                sqlCNX.Close();
                //Guardamos la consulta en qry
                qry = "INSERT INTO salidas(id_caja, id_trabajador, id_cliente, factura, total, fecha_registro, hora_registro, activo, id_usuario, descripcion) VALUES('"+lblid_caja.Text+"', '"+lblid_trabajador.Text+"', '"+lblid_cliente.Text+"','" + this.cmbfactura.Text + "', '" + this.txttotal.Text + "', '" + this.lblfecha_registro.Text + "', '" + this.lblhora_registro.Text + "', " + this.txtactivo.Text + "," + this.txtidusuario.Text + ", '"+txtdescripcion.Text+"')";
                //Asignamos la consulta al comando
                sqlCMD.CommandText = qry;
                //Asignamos la conexion al comando
                sqlCMD.Connection = sqlCNX;
                ...
                //Cerrar la conexion
                sqlCNX.Close();
                MessageBox.Show("Se a guardado la salida", "Sistema de inventario");
                this.habilitar();
                this.idsalida();
            }
        ...
        {
            lblid_cliente.Text = cmbcliente.SelectedValue.ToString();
        }
        ...
        private void Btnliberar_salida_Click(object sender, EventArgs e)
        {
            ...
            if (dgvproductos.Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    int id_salida = Int16.Parse(this.txtidsalida.Text);
                    ...
                    foreach (DataGridViewRow row in dgvproductos.Rows)
                    {
                        ...
                        int idproducto = Int16.Parse(row.Cells["Id_producto"].Value.ToString());
                        int cantidad_producto = Int16.Parse(row.Cells["Cantidad"].Value.ToString());
                        //Guardamos la consulta en qry para insertar los datos en detalle salida
                        qry = "INSERT INTO detalle_salida(id_salida, id_producto, precio_venta, cantidad, fecha_registro, hora_registro, activo, id_usuario, descripcion) VALUES('" + id_salida + "', '" + idproducto + "', '" + row.Cells["Precio_Venta"].Value.ToString() + "', '" + cantidad_producto + "', '" + this.lblfecha_registro.Text + "', '" + this.lblhora_registro.Text + "', '" + this.txtactivo.Text + "', '" + this.txtidusuario.Text + "', '"+txtdescripcion.Text+"')";
                        //Asignamos la consulta al comando
                        sqlCMD.CommandText = qry;
                    ...
                    //Consulta para actualizar el total de la tabla salidas
                    qry = "UPDATE salidas SET total=" + this.txtgran_total.Text + " where id_salida=" + id_salida + "";
                    //Asignamos la consulta al comando
                    sqlCMD.CommandText = qry;
                    //Asginamos la conexion al comando
                    sqlCMD.Connection = sqlCNX;
                ...
                {
                    MessageBox.Show("Error al liberar la salida", "Sistema de inventario");
                    MessageBox.Show(ex.Message.ToString());
                }
            }
            else
            {
                MessageBox.Show("No hay productos en esta salida", "Sistema de inventario");
            }
        }
        ...
        private void Btncancelar_salida_Click(object sender, EventArgs e)
        {
            ...
            respuesta = MessageBox.Show("Deseas cancelar la salida ", "Sistema de inventario", MessageBoxButtons.YesNo);
            if (respuesta == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    qry = "DELETE from salidas where id_salida =" + this.txtidsalida.Text + "";
                    //Asignamos la consulta al comando
                    sqlCMD.CommandText = qry;
                    //Asginamos la conexion al comando
                    sqlCMD.Connection = sqlCNX;
                    ...
                    //Invocamos el metodo cargar
                    cargar();
                    MessageBox.Show("Se a eliminado la salida con sus detalles", "Sistema de inventario");
                }
                catch (SqlException ex)
                {
                    MessageBox.Show("Error al liberar la salida", "Sistema de inventario");
